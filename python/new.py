...
commonWords = nlp(words1000)

wordsToRemove = list()
wordsRemoveFromAllowList = nlp('he she')

for remWord in wordsRemoveFromAllowList:
    for tok in commonWords:
        # print(remWord.text, tok.text, remWord.similarity(tok)) #, token.is_oov
        if remWord.similarity(tok) > 0.55:
            wordsToRemove.append(tok.text)

for word in wordsToRemove:
    words1000 = words1000.replace(" " + word + " ", "")

commonWords = words1000.toList()
print(commonWords)
